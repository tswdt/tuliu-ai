...
const templateList = [
  ...
  { id: "3", name: "3C场景图模板", category: "3c", platform: "京东", icon: Cpu, color: "bg-violet-50 text-violet-600", useCount: "1.5k", rating: "4.7", desc: "适合数码产品、电子配件等3C类目" },
  { id: "4", name: "服装主图+详情", category: "clothing", platform: "拼多多", icon: Shirt, color: "bg-blue-50 text-blue-600", useCount: "3.1k", rating: "4.6", desc: "适合男装、女装、童装等服装类目" },
...
export default function TemplatesPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-gray-900">模板中心</h1>
        <p className="text-sm text-gray-500 mt-0.5">选择适合你品类的模板，快速开始生成</p>
      </div>

      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
          <Input placeholder="搜索模板..." className="pl-9 h-9 text-sm" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map((t) => {
          const Icon = t.icon;
          return (
            <Card key={t.id} className="border-0 shadow-sm hover:shadow-md transition-shadow cursor-pointer group">
              <CardContent className="p-4">
                <div className={`h-28 rounded-lg ${t.color.split(" ")[0]} flex items-center justify-center mb-3 relative overflow-hidden`}>
                  <Icon className={`h-10 w-10 ${t.color.split(" ")[1]} opacity-60`} />
                  <div className="absolute bottom-2 right-2 flex items-center gap-1 bg-white/80 backdrop-blur-sm px-2 py-0.5 rounded text-[10px] font-medium text-gray-600">
                    <Star className="h-3 w-3 text-amber-400 fill-amber-400" />
                    {t.rating}
                  </div>
                </div>
                <h3 className="text-sm font-medium text-gray-900 group-hover:text-violet-600 transition-colors">{t.name}</h3>
                ...
                <div className="flex items-center justify-between mt-3">
                  ...
                  <span className="text-[11px] px-2 py-0.5 rounded bg-gray-100 text-gray-500">{t.platform}</span>
                </div>
              </CardContent>
            </Card>
          );
