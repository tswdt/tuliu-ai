    ...
    <div className="min-h-screen flex items-center justify-center bg-[#f5f5f7] px-4">
      <div className="w-full max-w-md bg-white rounded-2xl border border-[#e5e5e5] p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              />
              <label htmlFor="remember" className="ml-2 block text-[14px] text-[#666]">记住我</label>
            </div>
            <button type="button" className="text-[14px] text-[#1d1d1f] hover:underline cursor-pointer">忘记密码？</button>
          </div>

          <Button type="submit" className="w-full h-12 text-[16px] font-semibold" disabled={loading}>
            {loading ? "登录中..." : "登录"}
          </Button>
        </form>
        ...
        <p className="mt-8 text-center text-[14px] text-[#86868b]">
          还没有账户？{" "}
          <Link href="/register" className="text-[#1d1d1f] font-medium hover:underline">立即注册</Link>
        </p>
      </div>
    </div>
  );
